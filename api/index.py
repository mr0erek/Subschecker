...
# REDIRECT_URI = "https://subschecker.vercel.app/api/callback"
...
CLIENT_ID = os.getenv("CLIENT_ID")
CLIENT_SECRET = os.getenv("CLIENT_SECRET")
REDIRECT_URI = "https://subschecker.vercel.app/api/login"
CHANNEL_ID = "UCXnBZRpLD7QzcJsUKBF-cKw"  # Replace with your actual channel ID
...
# Create handler using BaseHTTPRequestHandler for Vercel
class handler(BaseHTTPRequestHandler):
    def do_GET(self):
        # Create a buffer for response
        buffer = BytesIO()
        
        # Use Flask to handle the response
        with app.test_request_context(self.path):
            response = app.full_dispatch_request()
            buffer.write(response.get_data())
